...
- **每个目标 = 一个独立数据源**(用户 2026-06-21 锁定:目标之间**不耦合**,各算各的;复合/加权是考核侧指标树的事,report 不做)。
- **report barrel 暴露只读 port**(按 goalKey 取,**不在考核侧重新指定筛选/聚合** —— 目标已自带筛选+统计+目标):
  `queryGoal({ reportTaskId, goalKey, orgIds? }) → Map<orgId, { actual, rate, met, grouped, groups? }>`
  实现 = 复用 `computeGoalProgress`(已逐目标独立),按 `target.orgId` 索引;一次调用只返回**一个目标**的各单位值。
- **assessment 数据源** `report.query`:per 指标存 `{ reportTaskId, goalKey, field:'rate'|'met'|'actual' }` —— 选一个报送任务 + 一个目标 + 取哪个量(完成率→`rate` 喂 proportional;达标 0/1→`met` 喂 binary;统计值→`actual` 喂 rank/minmax)。`field` 决定 effectiveOutputType(**唯一需碰 `isInputCompatible` 之处**,集中一处)。同一报送任务的多个目标 = 各自挂到**不同**考核指标,互不影响。
- **党组织考核对象 → 行政单位采购额**:`getLinkedAdminOrgs(partyOrgId)`(PartyAdminLink)换算,1:1 直取 / 1:N 求和;缺失 → UI 提示「未关联行政机构」。
- **前端 `ReportQueryEditor`** 挂进 `LeafConfigPanel`:选任务→选目标→取哪个量→预览各单位值(`POST /assessment/report-query/preview`,不落库)。
...
5. **★目标抽象成通用「报送明细查询工具」(2026-06-21,跨任意报送类型复用)**:
   - **模型升级**:`dim/dimValue`(写死扶贫列)→ **`GoalCondition[]`**(左侧多条件 AND;每条 `{col, values[]}`:col∈values 精确多值 / col 含 values 文本包含;values 空=全部)+ amount 的 **`metricCol`**(对哪个数值列求和)。presence 改为「存在命中条件的明细」(或 `fieldCode` 头字段)。旧 `dim/dimValue` 自动兼容(`normalizeConditions` 转 conditions)。
   - **★列按任务字段派生** `deriveGoalColumns(fields)`(前后端镜像):`catalog_pick`→快照子列(分部分/产品名/推荐单位/产地/清单供应商/销售方/规格,映射结构化列)、`select/text`→dim(走 `extra:<code>`)、`number`→metric(`role=amount`→`amount` 源·元/分;其余→`extra:<code>`·原值)、头层 `feeSource`→dim。引擎按 `col.source`(`col:*`/`amount`/`extra:*`)统一取值。**换报送类型(培训/台账/党员发展…)= 字段变,引擎零改**。
   - **明细持久化放宽**:`mapLines` 空行判定 `!hasProduct && !hasAmount` → `… && !otherContent`(其余列也都空才算空)。**纯文本+下拉报送(无金额无产品)也能存行**;扶贫行为不变(空行仍跳)。
   - **前端**:`ReportGoalEditor` 重做为**左条件 / 右目标**分栏(条件列下拉=派生 dim、值=精确多选下拉(分部分走 catalog 分类/费用来源走字段选项)或文本包含 chips;amount 多数值列时出「对哪列求和」选择器)。
   - 验证:门禁双绿;后端 e2e(① 通用报送:`数量`/`金额`求和 + `类别`select-presence,全走 extraJson ② 纯文本+下拉:存 2 行+空行跳+select/text presence ③ 扶贫派生列照旧 ④ 多条件 AND / 多值 OR / 文本含 / 旧模型兼容)+ 浏览器(派生 8 列、聚合选择器自适应、0 console error)。⚠ 连删导出时 Vite 热更新缓存会卡(报 `GOAL_COLS` 旧错)→ 重启 Vite 或硬刷新。
   - **大便宜**:这套「筛选+聚合」=考核取数口 `report.query` 的同一套地基,下一步②直接复用。
6. **★目标重构为三栏「筛选→统计→目标」+ 分组轴(2026-06-21,用户梳理后定稿)**:
   - **砍掉「类型(金额达标/是否有内容)」与「逐单位/全单位统一」模式**——用户研判后均可去:金额达标=求和+达到≥、是否有内容=计数+有内容>0;「统一」从模式降级为目标值表的「填满」按钮,**所有派发单位永远列出**(`perUnitGoals = targetOp!=='exists'`)。
   - **左·筛选**:`GoalCondition[]` → **单层分组** `groups:GoalGroup[]{op,conditions[]}` + `groupOp`(组间 且/或)。组内多条件 op 连、组间 groupOp 连、值内多值=或。能表达 `(福利费 且 第一部分) 或 (工会经费)`。旧扁平 `conditions[]`→单组、旧 `kind`→agg/op,`normalizeGoals` 全兼容。
   - **中·统计**:`agg=sum|avg|count`(文本/维度列只能 count)+ `metricCol`(空=命中行数)+ **`groupBy`/`grain`(分组轴)**:按日期列(年/季/月)或维度列值分堆,每堆各算一个 S。`deriveGoalColumns` 加 **`role='date'`** 列(头层 `purchaseDate` 注入每行 `structured.purchaseDate` + 明细 date 列)。
   - **右·目标**:`targetOp=gte|lte|exists`(gte/lte 需逐单位目标值,exists 不需)+ 分组时 `perGroupMode=all|atLeast`(+ `minGroups`/`expectedGroups`,季默认4/月12)。**「每季度至少2条」= 不筛 · 计数+分组季度 · 达到≥2+每组都要**,完成情况返回 `groups[]` 每季度的数 + 达标组数/期望组数 + 完成率。
   - **契约**:`GoalProgressItem` 改 `{op,grouped,money,actual,target,rate,met,groups[]}`(去 kind/actualAmount)。前端 `ReportGoalEditor` 三栏重写、`ReportGoalProgress` GoalCell 按 grouped/exists 分支 + 分组明细 chips。
   - 验证:门禁双绿(后端 0/0/0、前端 0/0);**引擎纯逻辑 e2e 16/16**(达到≥/有内容/每季度≥2分组含 Q1:3✓Q2:2✓Q3:1✗→50%未达/四季达标/组间或/旧两栏兼容)+ 浏览器(向导走到目标设定:三栏渲染、分组依据含「购买日期」、选日期→冒粒度+范围+期望分组数、口径实时、2 条件+组内且+加分组+组间或、34 单位目标值表、0 console error)。
7. **★去掉右栏「目标·是否达标」—— report 只给「目标 + 实际」,达标判断/给分归考核(2026-06-21,用户拍板)**:
   - **编辑器从三栏砍成两栏(筛选 + 统计)**:删 `targetOp`(达到≥/不超过≤/有内容>0)、`perGroupMode`/`minGroups`/`expectedGroups`。目标只产「实际值 S」+ 逐单位「目标值」(参考)。
   - **完成情况改中性展示**:`GoalProgressItem` = `{key,label,grouped,money,actual,target,rate,groups[]}`(去 `op`/`met`;`GoalGroupStat` 去 `met`)。`rate` = 实际/目标(中性),无「达标/未达」徽标;分组只列每堆数(灰 chip);所有目标都可设逐单位目标值(`perUnitGoals = goals`)。
   - **`queryGoal` 同步去 `met`/`op`**;`report.query` 取数 = `actual`/`rate`(+ `target`/`groups`),达标判断交考核。详情页目标编辑、向导、粘贴导入零改 import。
   - 理由:基层在报送下方写具体述职;**判断 + 给分集中到考核工具更顺畅**,report 当纯数据源(与「目标不耦合、复合在考核树」一致)。
   - 验证:门禁双绿(后端 0/0/0、前端 0/0);真实数据 goalProgress 中性结构(塔运司 goal_1 求和 ¥43,410/¥1,050,000·完成率 4.1%,**无 met/op**)+ 浏览器(编辑器仅筛选+统计两栏、无判据下拉、口径「…(实际值);达标判断在考核工具」;矩阵全页无「达标/未达」、只完成率;0 console error)。

### 下一步(明天接着做,按序)
- **② `report.query` 报送侧 port ✅(2026-06-21 已落地)**:`ReportService.queryGoal(taskId, goalKey)` → 各单位 `{ orgId, userId, name, actual, rate, met, target, grouped, money, groups[] }`(一次只出**一个**目标,目标独立不耦合);抽 `loadGoalRows` 与 `goalProgress` 共用;barrel + `ReportModule.exports` 已暴露(考核侧可直接 DI)。HTTP 预览口 `GET /reports/:id/goals/:goalKey/query`。e2e 8/8(结构/按 orgId 键/逐单位目标值透传/无提交→0未达/未知 goalKey→404/清理)。
  - ⚠ **一个报送任务 : 多个考核目标(1:N),目标之间不耦合**:考核指标点选「报送任务 + 哪个目标 + 取 actual/rate」(report 不再出 met——达标由考核判),同一任务的多个目标各挂**不同**考核指标,各算各的。goal 的稳定 `key`(goal_N)= 引用句柄。**复合/加权 + 达标判断一律在考核侧指标树做**;report 侧绝不合成、不判断。
  - **考核侧接入 ✅(2026-06-21 已落地)**:`assessment/data-sources.ts` 加 `report.query`(collection=business、ready)+ `effectiveOutputType(dataSource, sourceParams)`(report.query 随 `field`:rate→rate / actual→number;校验集中此一处)。叶子(`IndicatorNode`)加 **`sourceParams`**(存 `{reportTaskId, goalKey, field}`,区别于计分 `strategyParams`)。`AssessmentModule` import `ReportModule`、`AssessmentService` DI `ReportService`(DAG:assessment→report 单向,madge 0 环)。`computeRound` 对 report.query 叶子**自动取数**(`resolveReportQuery`:queryGoal → 各单位值 → 考核对象 ref 映射,**党组织对象经 `getLinkedAdminOrgs` 换算到行政单位匹配**,直接 orgId 命中优先 / 1:N actual 求和·rate 平均 / 未关联→null)覆盖到 raw。端点 `GET /assessment/report-query/sources`(有目标的报送任务+目标)+ `POST /assessment/report-query/preview`(各对象将取到的值)。前端 `LeafConfigPanel` 选「报送任务取数」→ `ReportQueryEditor`(任务/目标/取值 + 实时预览),计分工具按 effectiveOutputType 过滤。
    - 验证:门禁三端绿(后端 0/0/0、前端 0/0);**e2e**:preview 5/5(sources 列扶贫任务、actual 塔运司=43410 与 queryGoal 一致、rate 一致、未匹配→null)+ 保存校验 5/5(report.query+minmax 200、sourceParams 持久化、+grade_map(label) 400 拦截、rate+proportional 200)+ **党组织→行政 link 路径**(临时建「塔运司党委↔塔运司」→ 预览取到 43410,撤销)+ 浏览器(数据源含「报送任务取数」、选中出编辑器、任务下拉列扶贫任务、目标 3 个、计分工具按 number 过滤无 label 工具、预览渲染 35 对象)。
    - ⚠ **数据准备**:党建 track 考核对象是**基层党委**,报送任务派给**行政单位** → 需在「组织管理」给各基层党委建 `PartyAdminLink`(党委↔对应行政单位),report.query 才解析得到值(否则预览显示「无数据/未关联」)。目前仅个别 link 已建。
- **③ 扶贫复合分接考核(下一步)**:扶贫 = 大考核表里的**一个 2 分项**(拆成子树)。每个口径(总额完成率 / 福利费达标 / 工会达标 / 是否买第一部分)= 一个**独立报送目标**,在指标树各占一个子叶(各自 `report.query` 拉**一个**目标的 actual/rate),用考核**已有的权重 + 计分工具**在树里 rollup 成扶贫分 —— **不在 report 侧复合**。扶贫分 → 2 分用 `minmax`(或 `rank_tiers`),子项内部对 34 单位排名。⚠ 引擎现按整表总分排名;子项内部排名需把该子树 rollup 值当**一个 leaf 值**再 crossTarget 排名(叶子级排名已支持)。
- **④ 计分工具收敛**:弱化 `rank_linear` 等,整理到 6 核心。

### 已拍板(别再纠结)
- 目标设定 → **业务(report)**(DAG 红线:report 不反依赖 assessment)。
- **目标之间不耦合**(2026-06-21):每个报送目标各自独立计算,作为考核的**一个独立数据源**(按 goalKey 取 actual/rate);复合/加权在考核指标树做,report 绝不把多目标合成一个值。
- **report 不判断达标**(2026-06-21):目标只产「目标值 + 实际值(+ 中性完成率)」;**达标判断 + 给分全在考核工具**(基层在报送下方写述职)。编辑器只有 筛选 + 统计 两栏,无判据。
- 计分工具 → **考核(assessment)**。
