...
- **② `report.query` 报送侧 port ✅(2026-06-21 已落地)**:`ReportService.queryGoal(taskId, goalKey)` → 各单位 `{ orgId, userId, name, actual, rate, met, target, grouped, money, groups[] }`(一次只出**一个**目标,目标独立不耦合);抽 `loadGoalRows` 与 `goalProgress` 共用;barrel + `ReportModule.exports` 已暴露(考核侧可直接 DI)。HTTP 预览口 `GET /reports/:id/goals/:goalKey/query`。e2e 8/8(结构/按 orgId 键/逐单位目标值透传/无提交→0未达/未知 goalKey→404/清理)。
  ...
  - **⏭ 剩考核侧接入**:`assessment/data-sources.ts` 加 `report.query`(per 指标存 `{reportTaskId, goalKey, field}`)+ `LeafConfigPanel` 选任务/目标/取值 + `POST /assessment/report-query/preview` + 党组织对象→行政 `getLinkedAdminOrgs` 换算 + `field` 定 effectiveOutputType。
- **③ 接考核(assessment)**:扶贫 = 大考核表里的**一个 2 分项**(拆成子树)。每个口径(总额完成率 / 福利费达标 / 工会达标 / 是否买第一部分)= 一个**独立报送目标**,在指标树各占一个子叶(各自 `report.query` 拉**一个**目标的 rate/met),用考核**已有的权重 + 计分工具**在树里 rollup 成扶贫分 —— **不在 report 侧复合**。扶贫分 → 2 分用 `minmax`(或 `rank_tiers`),子项内部对 34 单位排名。⚠ 引擎现按整表总分排名;子项内部排名需把该子树 rollup 值当**一个 leaf 值**再 crossTarget 排名(叶子级排名已支持)。
