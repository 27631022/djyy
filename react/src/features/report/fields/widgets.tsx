...
import { PlusIcon, XIcon, UploadIcon, Loader2Icon, FileTextIcon, ImageIcon, SparklesIcon } from "lucide-react";
...
/**
 * 文件 / 图片填报控件:点选 → 走 storage 上传(ownerModule='report')→ 存 {id,name}[]。
...
/** 文件/图片字段:开启「上传后 AI 识别发票自动填表」开关(扶贫发票上传用)。 */
export function AiExtractToggle({
  field: f,
  patch,
}: {
  field: ReportField;
  ...
  return (
    <label className="flex items-start gap-2 rounded-md border border-[#dce4ef] bg-[#FBFBFC] p-2 cursor-pointer">
      <input
        type="checkbox"
        checked={!!f.aiExtract}
        onChange={(e) => patch({ aiExtract: e.target.checked || undefined })}
        className="mt-0.5"
      />
      <span>
        <span className="flex items-center gap-1 text-[12px] font-medium text-[#172033]">
          <SparklesIcon className="w-3.5 h-3.5 text-[var(--party-primary)]" />
          AI 识别发票自动填表
        </span>
        <span className="block text-[10px] text-[#9CA3AF] mt-0.5">
          开启后,承办人上传发票即可一键识别,自动填入发票号 / 日期 / 采买明细(走视觉模型,可配内网本地模型)
        </span>
      </span>
    </label>
  );
