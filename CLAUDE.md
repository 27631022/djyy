...
8. **现场大屏互动平台(`interactive`)** ★大方向 —— **第 0 期实时基座已交付(2026-07-07,见已完成:socket.io 网关 + 活动/游戏/局模型 + 双端游戏注册表 + 大屏/遥控扫码闭环)**。剩:第 1 期 你比我猜 / 第 2 期 摇一摇(触屏连点)/ 第 3 期 抢答器 / 第 4 期 幸运大转盘·摇奖(其它游戏慢慢加=注册表双端各一文件)。**加游戏 = `backend/src/interactive/games/<type>.game.ts` + `react/src/features/interactive/games/<type>.tsx` + 双端 registry 各一行**
