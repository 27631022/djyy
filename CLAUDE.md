...
- **(2026-06-09)企业虚拟展厅 P1:后端 exhibition 模块 + 美观大气 3D 客户端**(规格 v2 修订见 docs/specs/2026-06-07-virtual-exhibition-hall.md 第 15 节;用户三方向=美观大气 / 2D拖拽建厅 / 后台内容编辑,本期落①打底③):
  ...
  - **「局域网访问不到」根因记录(2026-06-10)**:本机三网卡全被 Windows 识别为 **Public 网络且无 node 入站规则 → 防火墙拦一切入站**。已加规则 `djyy-frontend-5173`/`djyy-backend-3001`(TCP Allow,Profile Any)。另:本机自测 10.x 网段会被 **xray_tun 代理劫持返回 503**(局域网其他设备不受影响,经 172.20.10.x WLAN IP 自测正常);多网卡时给用户的地址要按其设备所在网段给。
